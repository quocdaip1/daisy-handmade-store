...
export function CheckoutPage() {
  ...
  const checkCoupon = async () => {
    ...
    try {
      ...
      setCouponMessage(`Mã ${result.code} hợp lệ. Số tiền giảm sẽ được cập nhật vào tổng thanh toán.`)
    } catch (couponError) {
  ...
  return (
    <section className="modern-checkout">
      ...
      <header className="checkout-heading"><p className="eyebrow">Daisy Handmade Store</p><h1>Hoàn tất đơn hàng</h1><p>Kiểm tra thông tin và tổng thanh toán trước khi đặt hàng.</p></header>
      ...
      <form className="checkout-layout" onSubmit={handlePreview} noValidate>
        <div className="checkout-form-column">
          ...
          <section className="checkout-panel"><div className="checkout-panel-title"><span>02</span><div><h2>Địa chỉ giao hàng</h2><p>Vui lòng nhập đầy đủ thông tin để chúng tôi giao hàng chính xác.</p></div></div><div className="checkout-fields two-columns">
            ...
            <label className="full-field"><span>Ghi chú cho đơn hàng <i>(tùy chọn)</i></span><textarea rows={3} placeholder="Ví dụ: Giao hàng giờ hành chính, gọi trước khi giao..." value={formData.note} onChange={(event) => updateField('note', event.target.value)} /></label>
          </div></section>

          <section className="checkout-panel"><div className="checkout-panel-title"><span>03</span><div><h2>Ưu đãi</h2><p>Nhập mã ưu đãi để nhận giảm giá cho đơn hàng.</p></div></div><div className="voucher-box"><label htmlFor="checkout-coupon">Mã ưu đãi</label><div><input id="checkout-coupon" value={couponCode} aria-invalid={Boolean(errors.coupon)} onChange={(event) => { setCouponCode(event.target.value); invalidatePreview(); setErrors((current) => ({ ...current, coupon: undefined })) }} maxLength={50} /><button type="button" onClick={() => void checkCoupon()} disabled={!couponCode.trim() || isCheckingCoupon}>{isCheckingCoupon ? 'Đang kiểm tra' : 'Kiểm tra'}</button></div>{couponMessage ? <small className="success">{couponMessage}</small> : null}{errors.coupon ? <small className="invalid">{errors.coupon}</small> : null}</div></section>

          <section className="checkout-panel"><div className="checkout-panel-title"><span>04</span><div><h2>Phương thức thanh toán</h2><p>Hiện tại chúng tôi hỗ trợ thanh toán bằng chuyển khoản ngân hàng.</p></div></div><div className="payment-options"><label className="selected"><input type="radio" name="payment" checked readOnly /><span className="payment-icon">▣</span><div><strong>Chuyển khoản ngân hàng</strong><small>Thông tin chuyển khoản và mã QR sẽ hiển thị sau khi bạn xác nhận đặt hàng.</small></div><i>✓</i></label></div></section>
          {error ? <p className="checkout-error" role="alert">{error}</p> : null}
        </div>

        <aside className="checkout-summary"><h2>Tóm tắt đơn hàng</h2><div className="checkout-products">{(activePreview?.items ?? cartItems.map((item) => ({ product_id: item.product.id, name: item.product.name, quantity: item.quantity, unit_price: item.product.price, line_total: item.product.price * item.quantity }))).map((item) => { const cartItem = cartItems.find((candidate) => candidate.product.id === item.product_id); return <div key={item.product_id}><span>{cartItem?.product.images[0] ? <img src={cartItem.product.images[0]} alt="" /> : '✿'}<b>{item.quantity}</b></span><p><strong>{item.name}</strong><small>{cartItem?.product.material}</small></p><em>{formatCurrency(item.line_total)}</em></div> })}</div><div className="summary-lines"><div><span>Tạm tính ({totalItems} món)</span><strong>{formatCurrency(displayedSubtotal)}</strong></div><div><span>Giảm giá</span><strong>{displayedDiscount > 0 ? `-${formatCurrency(displayedDiscount)}` : couponCode.trim() && !activePreview ? 'Đang chờ xác nhận' : 'Chưa áp dụng'}</strong></div><div><span>Phí giao hàng</span><strong>{formatCurrency(activePreview?.shipping_fee ?? 0)}</strong></div></div><div className="checkout-grand-total"><span>Tổng thanh toán</span><strong>{formatCurrency(displayedTotal)}</strong></div>{activePreview ? <button type="button" className="button button-primary checkout-submit" disabled={isCreatingOrder} onClick={() => void handleCreateOrder()}>{isCreatingOrder ? 'Đang đặt hàng...' : 'Đặt hàng'} <span aria-hidden="true">→</span></button> : <button type="submit" className={`button button-primary checkout-submit ${isPreviewing ? 'is-loading' : ''}`} disabled={isPreviewing}>{isPreviewing ? <><i className="checkout-spinner" aria-hidden="true" />Đang tính tổng...</> : 'Xem tổng thanh toán'} <span aria-hidden="true">→</span></button>}<p className="checkout-terms">Giỏ hàng sẽ được cập nhật sau khi đơn hàng được đặt thành công.</p><div className="summary-assurance"><span>✓ Giá và số lượng sản phẩm được xác nhận trước khi đặt hàng</span><span>✓ Thông tin đơn hàng của bạn được xử lý an toàn</span></div></aside>
      </form>
    </section>
  )
}
